...
vector<string> readWordListFromFile(const string &filePath) {
  ...
  while (wordFile >> word) { // Nhung voi chuong trinh nay, doc tung word cung
                             // duoc Tuc ca 2 cach doc deu chay.
...
/***
    ...
    Returns:
        answer (string) : the lowercase word is in the position index of
wordList
***/
...
/***
    Args:
        incorrectGuess (int): a number that store the number of player's wrong
guess Returns: void
***/
...
/***
    Args:
        ...
        incorrectGuess (int): a number that stores the number of player's wrong
guess incorrectChars (string): a string that stores incorrect inputs of player
    Returns:
        void
***/
void processData(const char ch, const string &word, string &secretWord,
                 string &correctChars, int &incorrectGuess,
